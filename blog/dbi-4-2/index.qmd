...
- A new [vignette on Arrow support in DBI](https://dbi.r-dbi.org/articles/DBI-arrow.html)

- DBI specification is now version-controlled as Markdown files (previously has been assembled at package build time from various sources)

- Clarification of the command execution and data retrieval flow in the documentation

- New pkgdown documentation template <https://github.com/r-dbi/dbitemplate> that is used by all packages, thanks to Maëlle Salmon

Sundown of <https://relational.fit.cvut.cz>: relational.fit was a website that hosted various relational data models in a publicly accessible MySQL database.
We have secured a copy of the data and are working on a new hosting solution.

- FIXME: New blog

### Move to cpp11

The RSQLite, RPostgres, and RMariaDB packages are now using the cpp11 package for the glue between R and C++.
...
More on that below, thanks to Voltron Data for supporting this effort!
...
I consider myself a good developer, and the DBItest package is a prime example of this.
...
The DBItest package home to both the specification of the DBI package and a test suite for DBI backends.
...
Code generation has proved to be a powerful tool to peel off abstraction layers, for instance in the duckplyr project.
...
Unreleased version 1.3.1.9000 that avoids the deprecated `mysql_ssl_set()` function, pending discussion.
...
FIXME: Add figure from Excalidraw.
...
If your server supports this, enable fast loading by passing `load_data_local_infile = TRUE` to `dbConnect()`.
