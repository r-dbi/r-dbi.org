...
In this first part, we will give a glimpse on the new adbi package, summarize recent developments in {DBI} and related packages.
...
![Data flow from libpq to consumers](dbi-4.1-adbi.png){fig-alt="Space divided into two areas, R data frames at the left, Arrow at the right. Two concurrent data flows from libpq to DBI are shown: one via RPostgres through R data frames, one via adbcpostgresql and adbi through Arrow. The two flows merge at the DBI layer. From DBI, consumers can use dbGetQuery() to get data frames, or dbGetQueryArrow() to get Arrow streams."}
